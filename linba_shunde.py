...
path = 'H:/腋窝淋巴结最新/淋巴结顺德病例/'

xls = pd.read_excel(r'H:/腋窝淋巴结转移-数据划分.xlsx',
                        sheet_name='顺德病例-外部验证2')
    ...
    for temp_list_each in temp_list:
        ...
        if os.path.isfile(file_path + '/' + temp_list_each):
            ...
            if os.path.splitext(temp_path)[-1] == ('.png'or'.bmp') and os.path.splitext(temp_path)[-1] != '.xml':
                ...
                cv2.imencode('.jpg', img)[1].tofile('H:/linba_newProcessData_noxml/image/shunde/'+patient_name[:-4] + ".jpg")  # 英文或中文路径均适用
            ...
            if os.path.splitext(temp_path)[-1] == '.xml':    #自己需要处理的是.log文件所以在此加一个判断
                ...
                if(os.path.exists(temp_path.split('.xml')[0] + ".png")):
                    ...
                    print(temp_path," —— bmp/jpg is exist")
                    img = cv2.imdecode(np.fromfile((temp_path.split('.xml')[0] + ".bmp"), dtype=np.uint8), -1)

                cv2.imencode('.jpg', img)[1].tofile('H:/linba_newProcessData/image/shunde/'+patient_name[:-4] + ".jpg")  # 英文或中文路径均适用
                shutil.copyfile(temp_path, 'H:/linba_newProcessData/xml/shunde/'+patient_name[:-4] + ".xml")
...
np.savetxt('H:/shunde.txt',newclinic, delimiter=' ',fmt = '%s')  ### 将数据重新拍好顺序再统一保存起来
