...
path = 'H:/新辅助化疗/新辅助云南病例/'

xls = pd.read_excel(r'H:/新辅助化疗-数据划分.xlsx',
                        sheet_name='云南病例-外部验证')

dict1 = xls.loc[:,['姓名','年龄','病理类型（浸润性导管癌=0，浸润性小叶癌=1，其他类型=2）','cT分期(T1=1,T2=2,T3=3,T4=4)','ER（阴性=0，阳性=1）','PR（阴性=0，阳性=1）','HER2（阴性=0，阳性=1）','Ki-67（<20%=0，≥20%=1）','新辅助化疗后是否达到pCR（否=0，是=1）']]
...
def check_if_dir(file_path):
    ...
    for temp_list_each in temp_list:
        ...
        if os.path.isfile(file_path + '/' + temp_list_each):
            ...
            if os.path.splitext(temp_path)[-1] == ('.png'or'.bmp') and os.path.splitext(temp_path)[-1] != '.xml':
                
                hanzi = temp_path.rsplit('/')[-3]              
                patient_name = pinyin(hanzi)[:-9]  ## [:-9]是消除云南病例中名字后面的（2张）的字样
                save_name = temp_path.rsplit('/')[-1]

                img = cv2.imdecode(np.fromfile((temp_path), dtype=np.uint8), -1)

                cv2.imencode('.jpg', img)[1].tofile('H:/xinfuzhu_newProcessData_noxml/image/yunnan/'+patient_name[:-3] + ".jpg")  # 英文或中文路径均适用
            ...
            if os.path.splitext(temp_path)[-1] == '.xml':    #自己需要处理的是.log文件所以在此加一个判断
                ...
                if(os.path.exists(temp_path.split('.xml')[0] + ".png")):
                    ...
                    img = cv2.imdecode(np.fromfile((temp_path.split('.xml')[0] + ".bmp"), dtype=np.uint8), -1)

                cv2.imencode('.jpg', img)[1].tofile('H:/xinfuzhu_newProcessData/image/yunnan/'+patient_name[:-3] + ".jpg")  # 英文或中文路径均适用
                shutil.copyfile(temp_path, 'H:/xinfuzhu_newProcessData/xml/yunnan/'+patient_name[:-3] + ".xml")
...
check_if_dir(path)
np.savetxt('H:/yunnan.txt',newclinic, delimiter=' ',fmt = '%s')  ### 将数据重新拍好顺序再统一保存起来
