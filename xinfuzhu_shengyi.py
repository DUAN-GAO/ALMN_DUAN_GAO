    ...
    for temp_list_each in temp_list:
        ...
        if os.path.isfile(file_path + '/' + temp_list_each):
            ...
            if os.path.splitext(temp_path)[-1] == ('.png'or'.jpg') and os.path.splitext(temp_path)[-1] != '.xml':
                ...
                cv2.imencode('.jpg', img)[1].tofile('H:/xinfuzhu_newProcessData_noxml/image/shengyi/'+patient_name[:-3] + ".jpg")  # 英文或中文路径均适用
            ...
            if os.path.splitext(temp_path)[-1] == '.xml':    #自己需要处理的是.log文件所以在此加一个判断
                ...
                cv2.imencode('.jpg', img)[1].tofile('H:/xinfuzhu_newProcessData/image/shengyi/'+patient_name[:-3] + ".jpg")  # 英文或中文路径均适用
                shutil.copyfile(temp_path, 'H:/xinfuzhu_newProcessData/xml/shengyi/'+patient_name[:-3] + ".xml")
